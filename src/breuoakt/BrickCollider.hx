...
class BrickCollider {
    ...
    private static var SIZE_TOLERANCE = 4;
    ...
    public function collide(ball:Ball) {
        ...
        for (i in 0...numCollidedBricks) {
            ...
            if (brick.x + Brick.WIDTH * 0.5 - SIZE_TOLERANCE < ball.x - Ball.WIDTH * 0.5) {
                brickLeftIndex = brickIndex;
            } else if (brick.x - Brick.WIDTH * 0.5 + SIZE_TOLERANCE > ball.x + Ball.WIDTH * 0.5) {
                brickRightIndex = brickIndex;
            }

            if (brick.y + Brick.HEIGHT * 0.5 - SIZE_TOLERANCE < ball.y - Ball.HEIGHT * 0.5) {
                brickAboveIndex = brickIndex;
            } else if (brick.y - Brick.HEIGHT * 0.5 + SIZE_TOLERANCE > ball.y + Ball.HEIGHT * 0.5) {
        ...
        if (brickAboveIndex < 0 && brickRightIndex < 0 && brickBelowIndex < 0 && brickLeftIndex < 0) {
            ball.velocity.x = -ball.velocity.x;
            ball.velocity.y = -ball.velocity.y;
        }
